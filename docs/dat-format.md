...
- [x] Bonus values (+7, +13) — **CLOSED.** Type-167 localization names provide values (wired). FID-identity confirmed NOT value-discriminating. Binary stat resolution is text-only. Different +values might reference different type-167 FIDs. Also check if any OTHER entry types (type-414 sub-chains, type-53 non-byte-68 fields) encode the value.
- [x] Weapon critical range ("19-20/x2") — **CLOSED: wiki-only.** Not in properties, effect chain, or localization. Wiki `crit` field is the source. Check if type-414 feat effect entries (e.g., "Improved Critical") encode critical data. Check if localization names on any effect type contain critical range text.
...
- [x] Item enhancement_bonus via effect_ref FID — **DEFERRED: wiki provides via `{{Enhancement bonus|w|2}}`.**
- [x] Item augment slot types via effect_ref FID — **DEFERRED: wiki provides via `{{Augment|Color}}`.**
...
- [x] Replace race_ability_bonuses seed with wiki scraper — **INVESTIGATED.** The Races page (ddowiki.com/page/Races) has a structured sortable chart with separate columns for "Innate Racial Ability Modifiers" and "Racial Enhancement Ability Modifiers." 28 races parsed, 21 with innate mods, 15 with enhancement mods. Issues: Unicode minus (U+2212) needs handling, complex entries ("your choice of +1 any stat"), `&nbsp;` in names. **TODO:** wire Races page parser into seed generation, add innate vs enhancement distinction to schema. Current seed retained but incomplete (missing +1 mods, missing enhancement mods).
- [ ] Add alignment restrictions to classes and iconic races — DDO classes and iconic races have alignment restrictions (e.g., Paladin must be Lawful Good, Monk must be Lawful). Schema needs alignment columns.
...
- [x] Icon columns added to items, feats, enhancements, spells — all from wiki image filenames. Wiki icon URL pattern: `https://images.ddowiki.com/<filename>` (e.g., `WpnIcon_BattleAxe01.jpg`). Coverage: items 88%, feats 98%, enhancements 100%, spells 99%.
- [x] ~~Investigate binary icon extraction~~ — **EXHAUSTIVELY INVESTIGATED, confirmed not extractable.** Tried: (1) DDS/PNG/JPG magic in all .dat files — none found; (2) Turbine DXT format (offset 8) — found 2,488 world textures, no UI icons; (3) raw RGBA interpretation of client_general.dat 0x01 entries — random noise; (4) PE resource extraction from dndclient.exe — only DDO logos; (5) UI skinning system (ArtAssetID) — only covers chrome, not content icons; (6) DungeonHelper reads icons from game memory at runtime, not files. Community sources confirm icons are screenshots, not extracted assets.
