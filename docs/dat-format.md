...
- [ ] Class/race body decoding — **extensively investigated, partially decoded**:
  ...
  - 128-byte shared preamble: identical across all classes. Contains VLE Pascal strings "on"/"off" (toggle states). Uses DDO-extended VLE type tags (type 8193 = 0x2001) that the Turbine decoder doesn't support.
  - Class-specific section: varies wildly (18B Rogue, 190B Sorcerer/Wizard, 5218B Ranger). Sorcerer vs Wizard differ in only 8 bytes (4 pairs of class-specific IDs). Contains floats 40.0/60.0 (level thresholds?).
  - Ranger ability blocks: 50 blocks at 92 bytes each, mirrored in two halves (blocks 0-24 = blocks 25-49). Per block: circle assignment (0-4), level threshold float, flags. Circle distribution matches wiki Ranger spell counts.
  - **Blocked on:** (a) DDO extended VLE type tags (8193+) prevent full preamble decode, (b) 0x07XXXXXX values in blocks are packed parameters not file IDs (not in archive), (c) cannot map blocks to specific spells without runtime data.
  - **Decodable now:** class index, ability block count, per-block circle/level/flags. **Not decodable:** specific spell-to-block mapping, base class stats (hit die, BAB, saves) in the opaque preamble.
- [ ] Feat flag discovery — **found separate property keys encoding feat types** (562 matched feats). Active: keys 0x10000829 (100%, N=7) and 0x10002878 (100%, N=10) appear only on active feats. Stance: key 0x100024D1 (90%, N=10) and 0x10000771 (85%, N=26). Free: key 0x100040FB (100%, N=7). Epic destiny: key 0x100033C9 (50%, N=8). Metamagic: wiki only has 1 metamagic feat ("Empower Healing Spell") — wiki `metamagic=yes` flag is barely populated, but binary has 100+ metamagic-related entries ("Efficient Metamagic: Empower", "Improved Empower Spell", etc.). **Needs:** (a) verify flag keys with more examples, (b) wire presence/absence of these keys into feat parser to set is_active/is_stance/is_free from binary, (c) fix wiki metamagic flag scraping (base feats "Empower Spell", "Maximize Spell", etc. should have metamagic=yes), (d) the original bitmask key 0x1000088E may encode a different taxonomy (ability type/category).
...
- [ ] Item effect_ref chain decoding — items have 28+ effect_ref slot keys (0x10000919, 0x10001390, etc.) pointing to 0x70XXXXXX effect entries. The chain structure may encode augment slots, weapon properties (damage/critical), armor properties (AC/max dex), and other fields currently listed as wiki-only. Investigate: (a) do specific effect_ref slots consistently map to specific property types (e.g., slot 3 = augment config, slot 7 = weapon damage)? (b) do the 0x70 effect entries referenced by augmented items differ from non-augmented items in a way that encodes slot color/count? (c) trace the full effect chain for a known item (e.g., a weapon with specific damage/crit) to see if weapon stats are in the effect entries.
- [ ] Item complex sub-structure decoding — the Type-2 complex-partial body of item entries (beyond the dup-triple properties) may contain additional data. Items with DID=2 have VLE-encoded bodies that the current decoder can't fully parse. Investigate whether weapon damage ("1d6+4"), critical range ("19-20/x2"), augment slot configuration, or material type are encoded in the complex body rather than the dup-triple properties.
...
- [ ] Spell ref slots 3-14 and 17+ — non-stat ref slots may encode mechanically relevant data (AoE radius, range in feet, targeting type, duration ticks) as numeric parameters that don't match wiki text fields. Current correlators only checked wiki text fields; need to survey raw slot values for meaningful numeric ranges.
- [ ] Localization sub-entry types — 13 types cataloged (Name through QuestObjective). There may be additional sub-entry refs carrying ability descriptions, enhancement rank text, or other build-relevant strings not yet discovered.
- [ ] DDO extended VLE type tags — type tag 8193 (0x2001) found in class preambles. Understanding DDO's extensions to the Turbine VLE format would unlock class preamble decoding (hit die, BAB, saves) and potentially all Type-2 complex-partial entries across all namespaces.
- [ ] Type-1 behavior script investigation — 6,838 entries in 0x07 namespace. Primarily quest triggers and NPC logic, but may also encode ability/feat behavior with numeric parameters (damage formulas, DC calculations, cooldown values). Worth surveying for build-relevant data.
- [ ] Item sub-schema clusters — runearm items have 0x0D88XXXX packed refs and type-specific properties that may encode charge levels, max charge, and runearm-specific stats (build-relevant for Artificers). Sheet Music items have 40+ zero-constant slots that may encode instrument/perform mechanics.
