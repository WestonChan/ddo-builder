...
- [ ] Feat bitmask-to-flag mapping — **re-ran with 40 matched feats (out of 291 name matches, only 40 carry the bitmask key)**. Bit 25 strongest signal: 67% active when set vs 19% clear, 0% passive vs 78% — possible active flag but N=3. Bit 18 correlates with passive+free (88%/50%). N=40 too noisy for confident assignment. **Needs:** either find MORE property keys that encode feat flags (separate keys for passive/active/stance rather than one bitmask), or increase binary coverage to get more matched feats with the bitmask key.
- [ ] Feat bonus_classes discovery — search for class ID refs in feat property arrays. **Blocked on:** only 40 matched feats carry relevant property keys.
- [ ] Augment slot format investigation — **dup-triple arrays don't encode augment slots**. Need: investigate whether augment slot types are encoded in 0x70 effect entries, or in the complex-partial body of item entries (not just the dup-triple properties). May require probing items with known augment configurations.
- [ ] Composite field survey across non-spell entry types — check whether 0x70 effect "mechanism classifier" stat_def_ids (1254, 1440, 551, 2114) pack stat + modifier type in sub-bytes. Check 0x07 class entry VLE bodies for packed fields. Initial byte decomposition of feat bitmask showed no clean sub-byte structure, but effect entries are a different format.
