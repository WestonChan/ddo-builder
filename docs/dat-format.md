...
- [x] Decode packed spell ref slot bit layout — **DISPROVEN**: byte-level decomposition of slots 15-16 and 27-29 shows no clean sub-byte structure. Values are opaque template pointers, not packed composites. The save/SR/range correlations at 90%+ were spurious — driven by school being a strong predictor of save type and SR (e.g., Evocation = Reflex + Yes SR). Only school is independently decodable (via the hash lookup table already wired). Save, SR, and range remain wiki-only.
- [x] Survey for composite/packed fields — **not applicable**: the spell packed-field hypothesis was disproven. The large u32 values in spell ref slots are template/config pointers, not bitfields. The same is likely true for other entry types (feat bitmask 0x1000088E has 144 unique values with no clean byte-level decomposition either). No further composite field survey needed.
- [x] Spell field correlation for remaining fields — components NOT found (no slot correlation), target NOT found (spurious 90%+ across ALL slots from Self-spell base rate), duration/damage_types/metamagics NOT found in ref slots. **Confirmed wiki-only:** components, target, duration, damage_types, metamagics.
- [x] Investigate 0x47 spell entry bodies — 88% are 0-3 bytes. 32 matched entries with bodies >10 bytes contain structured dup-triple-like data (stat_def_id pairs encoding additional spell parameters: damage dice, duration formulas). Bodies are an extension of the ref list for complex spells, not a separate encoding. Parsing would require dedicated body decoder but low priority since wiki covers these fields.
- [ ] Class/race binary entry decoding — **deep probe complete**: all class DID=2 entries share identical body header (`01 01 1B 00 00 00 XX`, XX=class index). Ranger (5,346B) has 50x key 0x100072CD (class ability marker), 60x 0x07 game object refs (granted spells), 50x 0x70 effect refs, 8x 0x0A localization strings. Bodies use VLE property streams, not dup-triple. **Blocked on:** Type-2 complex-partial VLE stream decoder improvements. Ranger's 60 spell refs would yield the full class spell list if decoded.
- [x] Feat boolean flag bit decomposition — key 0x1000088E is a genuine 31-bit bitfield. Bit 2 (786 entries) and bit 18 (807 entries) are the most common flags. 61-feat wiki sample was too small for bit-to-flag mapping. **Blocked on:** larger wiki-to-binary feat name overlap (currently only 61/706 match). Could retry with improved name normalization or partial matching.
- [ ] Feat bonus_classes discovery — search for class ID refs in feat property arrays matching wiki bonus_classes lists. **Blocked on:** same 61-feat match limitation.
- [x] Augment slot discovery — scanned all item dup-triple entries for small-int array properties. **Result:** No array-valued keys found encoding augment slot types. Augment slots are NOT in the dup-triple property format — they may be in effect entries or a separate data structure. Confirmed wiki-only.
- [x] Set membership discovery — group_ref (0x10000A48) is NOT set membership. Probed 16,400 entries across 17 values: groups contain quests, NPCs, effects, and items mixed together (e.g., 0x0008AB00 has 12,071 entries including sagas and NPCs). Zero wiki set_name matches. Set membership is confirmed wiki-only.
