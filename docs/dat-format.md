...
- [ ] Class/race body decoding — **exhaustively investigated, blocked on proprietary template system**:
...
- [ ] Enhancement binary property decoding — **14,276 entries, 714 names have multiple entries (stub + detailed).** Confirmed: 0x10001A4C = **max_ranks**, 0x1000191A = **level/cap** (always 20), 0x10003BE4 = **cost metric**. Tree bodies at 0x07 (Air Savant=11,226B) scanned: contain NO enhancement FIDs — same opaque format as class entries. Tree→enhancement linkage NOT in binary. Property ref chains (0x10001A4A: 74 values, 0x10001A4B: 218 values) may encode enhancement prerequisites. Needs: (a) wire max_ranks, (b) investigate property chains as prereqs.
