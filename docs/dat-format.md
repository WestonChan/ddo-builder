...
- [ ] Class/race body decoding — **block structure decoded**: Ranger has 50 ability blocks at 92 bytes each. Layout: [8 zeros][0x100072CD marker][flags][float 1.0 x2][u16 decreasing index][spell_circle:u8]. Trailing 4 bytes are float level thresholds (0/10/20/-20/-10). Format IS parseable but **blocked on:** 0x07XXXXXX ability refs (0x07430101, 0x073F8000) don't resolve to named strings — need to probe these entries to identify which spells each block grants. Would yield full class spell lists with level requirements per circle.
