...
- [ ] Effect type 414 decoding — **census complete**: 46 entries, all 425B, DID=2, 0 refs. Named: "Improved Search I", "Tireless Rage" — feat/ability effect definitions. VLE-encoded body (same DDO format, blocked on template decode). Always referenced by effect_ref_2 slot. **Needs:** decode the 425-byte body layout for build-relevant data (ability bonuses, conditions, parameters).
...
- [ ] Effect_ref slot 28 augment investigation — slot 28 (effect_ref_28_06E1) references type-95 (65 items) and type-175 (39 items) entries. Type-175 are the magnitude tables. Type-95 is a new undecoded type. **Needs:** decode type-95 entries and check if slot 28 presence/values correlate with augment slot count or color.
- [ ] Effect type 62 decoding — 104 items reference type-62 entries via effect_ref_16_3EF4. Unknown type — needs census and layout decode.
