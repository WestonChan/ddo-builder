...
- [x] Item effect_ref chain decoding — **traced full chains for known weapons/armor with augments**. Most items have 1-4 refs populated (not 28+). Chain encodes enchantment bonuses (type-17 mechanism classifiers, type-53 magnitude bonuses) and special properties (e.g., type-414 "Improved Critical" at 425 bytes). **Weapon damage, critical range, and augment slots are NOT in the effect chain.** Shared effect 0x700027E1 (Sprint Boost, stat 2053) on 1,086 items. New discovery: effect type 414 is an undiscovered complex effect with embedded localization text — may warrant separate investigation.
