...
- [ ] Apply FID-identity approach to ALL template-blocked tasks — the stat discovery showed that template CONTENT is shared but template FID is context-specific. Every previous "dead end" where we found identical template content should be re-examined via FID discriminant analysis:
  - **(a) bonus_type resolution** — do Enhancement vs Insightful vs Quality items reference different effect FIDs? Would crack the BONUS_TYPE_CODES wall.
  - **(b) augment slot detection** — do augmented items reference specific effect FIDs not found on non-augmented items? The slot 28 (type-95/175) investigation checked entry types but not FID identity.
  - **(c) weapon damage/crit** — do weapons with different damage dice reference different FIDs? Different base damage could map to different effect_ref FIDs.
  - **(d) mechanism classifier FIDs** — sid 1254/1440/551/2114 entries are content-identical, but 31,750 entries have thousands of unique FIDs. The FID might encode the SPECIFIC stat mechanism (Strength vs Constitution) even though the content says "generic mechanism."
  - **(e) type-26 augment marker FIDs** — all content-identical per sid, but do augmented items reference different type-26 FIDs than non-augmented? Could reveal augment encoding.
  - **(f) type-167 sub-effect FIDs** — 37K content-identical entries, but do enhancement rank effects reference different FIDs per rank? Would unlock enhancement rank bonus values.
  - **(g) type-62 pattern FIDs** — 3 unique patterns across 188 entries. Which items use which of the 3 patterns? The pattern may encode item category or augment type.
  - **(h) type-414 sub-chain FIDs** — feat effect entries contain chains of sub-effect FIDs. Do different feats' sub-effect FIDs discriminate by feat bonus type?
