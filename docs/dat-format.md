...
- [ ] Spell body stat decoding — **cross-reference complete**: 2,963 entries with 76 stat_def_ids. **Stat 731** is the only useful per-spell variable: values 1/3/5/8 encode tick/effect count (Focusing Chant=1, Cometfall=3, Cloudkill=5, Energy Drain=8). Stat 708 is constant 17 (mechanism type code). Stats 950/947/1368 are config/template refs. Stats 943 is boolean flag. Need: (a) wire body dup-triple scanning for stat 731 (tick count) and existing stats 553/554/946, (b) determine if stat 731 tick count is useful for the build planner UI.
- [ ] Class/race VLE body decoding — **investigation found class entries at 0x07001694-98 with VLE property streams**. Ranger (5,346B) has 50x key 0x100072CD (class ability marker), 60x spell refs, 50x effect refs. All classes share body header `01 01 1B 00 00 00 XX`. Need: improve Type-2 complex-partial VLE stream decoder to parse nested structures. Would yield full class spell lists and granted abilities.
- [ ] Feat bitmask-to-flag mapping — **investigation confirmed key 0x1000088E is a genuine 31-bit bitfield** (bit 2: 786 entries, bit 18: 807 entries). But only 61/706 wiki feats matched binary by name — too small for bit-to-flag correlation. Need: improve feat name matching (partial match, alias resolution) to increase overlap, then re-correlate individual bits against wiki is_passive/is_active/is_stance/is_metamagic flags.
- [ ] Feat bonus_classes discovery — search for class ID refs in feat property arrays. **Blocked on:** same 61-feat name match limitation as bitmask task.
