...
- [ ] Feat flag discovery — **found separate property keys encoding feat types** (562 matched feats). Active: keys 0x10000829 (100%, N=7) and 0x10002878 (100%, N=10) appear only on active feats. Stance: key 0x100024D1 (90%, N=10) and 0x10000771 (85%, N=26). Free: key 0x100040FB (100%, N=7). Epic destiny: key 0x100033C9 (50%, N=8). Metamagic: only 1 wiki feat, no correlation possible. **Needs:** (a) verify with more examples, (b) wire presence/absence of these keys into feat parser to set is_active/is_stance/is_free flags from binary, (c) the original bitmask key 0x1000088E may encode a different taxonomy (ability type/category rather than simple boolean flags).
- [ ] Feat bonus_classes discovery — search for class ID refs in feat property arrays. 562 feats matched, investigation can proceed.
- [x] Augment slot format investigation — dup-triple arrays don't encode them. Wiki items JSON has 0 augment_slots populated (wiki parser issue, not binary). Binary investigation **blocked on wiki data** — need augment slot data in wiki items to cross-reference against binary properties. The wiki `augmentslot` template field may need parser fixes.
