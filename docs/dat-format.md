...
- [ ] Re-probe binary after wiki improvements — with metamagic feats now correctly flagged (9 feats, was 1), re-run feat flag correlation to check if any binary property key encodes metamagic status. Also check if the `active=yes` fix changes any other flag correlation results.
